...
| `&sortby`        | Field to sort the results by. For example `CreatedDate`, `LastChangedDate`, `Zip`, `Price`, etc.                                                                                                                    | `CreatedDate`                 |
...
| `&where`         | Generic filter property, provide a JSON string. For example ```&where=`{"SubStatus:!=": "6"}` ``` which will hide properties that have been sold. Note that valid JSON means double quotes!                         | (empty)                       |
