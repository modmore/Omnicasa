...
$category->set('category','Omnicasa');
...
foreach($snippetSource as $name => $options) {
    $snippets[$name] = $modx->newObject(\MODX\Revolution\modSnippet::class);
...
unset($snippets);
$modx->log(xPDO::LOG_LEVEL_INFO,'Packaged in snippets.');
...
$modx->log(xPDO::LOG_LEVEL_INFO,'Packaging in Dashboard Widgets...');
$widgets = include $sources['data'].'transport.dashboard_widgets.php';
if (empty($widgets)) $modx->log(xPDO::LOG_LEVEL_ERROR,'Could not package in widgets.');
$attributes = array(
    xPDOTransport::PRESERVE_KEYS => false,
    xPDOTransport::UPDATE_OBJECT => true,
...
$modx->log(xPDO::LOG_LEVEL_INFO,'Packaged in '.count($widgets).' widgets.'); flush();
